...
## 3. 분석 과정
앞서 부산광역시에 수소충전소가 추가로 필요하다고 말했다. 그렇다면 부산광역시 어디에 설치해야 하는지 적절한 위치를 알아내 보려고 한다. 일단 부산광역시에 설치된 기존 수소충전소 위치와 구별 수소자동차 등록수를 파악해 
...
```{python}
# 마커
m_2 = folium.Map(location = [35.1796, 129.0750], tiles='cartodbpositron', zoom_start = 10)

# 지도에 표시
folium.Marker([35.113698, 128.843072], tooltip = "(주)엔케이텍 서부산").add_to(m_2)
folium.Marker([35.145745, 128.988180], tooltip = "대도하이젠(주)수소복합충전소").add_to(m_2)
folium.Marker([35.328968, 129.201549], tooltip = "하이넷 부산 정관 수소충전소").add_to(m_2)
